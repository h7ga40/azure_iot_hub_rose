...
CONCRETE_IO_HANDLE socketio_esp_at_create(void* io_create_parameters);
void socketio_esp_at_destroy(CONCRETE_IO_HANDLE socket_io);
int socketio_esp_at_open(CONCRETE_IO_HANDLE socket_io, ON_IO_OPEN_COMPLETE on_io_open_complete, void* on_io_open_complete_context, ON_BYTES_RECEIVED on_bytes_received, void* on_bytes_received_context, ON_IO_ERROR on_io_error, void* on_io_error_context);
int socketio_esp_at_close(CONCRETE_IO_HANDLE socket_io, ON_IO_CLOSE_COMPLETE on_io_close_complete, void* callback_context);
int socketio_esp_at_send(CONCRETE_IO_HANDLE socket_io, const void* buffer, size_t size, ON_SEND_COMPLETE on_send_complete, void* callback_context);
void socketio_esp_at_dowork(CONCRETE_IO_HANDLE socket_io);
int socketio_esp_at_setoption(CONCRETE_IO_HANDLE socket_io, const char* optionName, const void* value);

/*this function will clone an option given by name and value*/
static void* socketio_esp_at_CloneOption(const char* name, const void* value)
{
    (void)name;
    (void)value;
    return NULL;
...
static void socketio_esp_at_DestroyOption(const char* name, const void* value)
{
    (void)name;
    (void)value;
}

static OPTIONHANDLER_HANDLE socketio_esp_at_retrieveoptions(CONCRETE_IO_HANDLE socket_io)
{
    OPTIONHANDLER_HANDLE result;
    (void)socket_io;
    result = OptionHandler_Create(socketio_esp_at_CloneOption, socketio_esp_at_DestroyOption, socketio_esp_at_setoption);
    if (result == NULL)
    {
        /*return as is*/
    }
...
{
    socketio_esp_at_retrieveoptions,
    socketio_esp_at_create,
    socketio_esp_at_destroy,
    socketio_esp_at_open,
    socketio_esp_at_close,
    socketio_esp_at_send,
    socketio_esp_at_dowork,
    socketio_esp_at_setoption
};
...
static int retrieve_data(SOCKET_IO_INSTANCE* socket_io_instance)
{
    ...
    while (received > 0)
    {
        received = esp_at_socket_receive(socket_io_instance->tcp_socket_connection, (char*)recv_bytes, ESP_AT_XIO_RECEIVE_BUFFER_SIZE);
        if (received > 0)
        {
...
static int send_queued_data(SOCKET_IO_INSTANCE* socket_io_instance)
{
    ...
    while (first_pending_io != NULL)
    {
        ...
        int send_result = esp_at_socket_send(socket_io_instance->tcp_socket_connection, (const char*)pending_socket_io->bytes, pending_socket_io->size);
...
static void close_tcp_connection(SOCKET_IO_INSTANCE* socket_io_instance)
{
    if (socket_io_instance->io_state != IO_STATE_CLOSED)
    {
        ...
        {
            esp_at_socket_close(socket_io_instance->tcp_socket_connection);
            esp_at_socket_destroy(socket_io_instance->tcp_socket_connection);
...
int socketio_esp_at_open(CONCRETE_IO_HANDLE socket_io, ON_IO_OPEN_COMPLETE on_io_open_complete, void* on_io_open_complete_context, ON_BYTES_RECEIVED on_bytes_received, void* on_bytes_received_context, ON_IO_ERROR on_io_error, void* on_io_error_context)
{
    ...
    {
        socket_io_instance->tcp_socket_connection = esp_at_socket_create(false);
        if (socket_io_instance->tcp_socket_connection == NULL)
        {
            result = MU_FAILURE;
        }
        ...
        {
            if (esp_at_socket_connect(socket_io_instance->tcp_socket_connection, socket_io_instance->hostname, socket_io_instance->port) != 0)
            {
                esp_at_socket_destroy(socket_io_instance->tcp_socket_connection);
                socket_io_instance->tcp_socket_connection = NULL;
                result = MU_FAILURE;
            ...
            {
                esp_at_socket_set_blocking(socket_io_instance->tcp_socket_connection, false, 0);
...
int socketio_esp_at_send(CONCRETE_IO_HANDLE socket_io, const void* buffer, size_t size, ON_SEND_COMPLETE on_send_complete, void* callback_context)
{
    ...
    else
    {
        ...
        {
            // Queue the data, and the socketio_esp_at_dowork sends the package later
...
const IO_INTERFACE_DESCRIPTION* socketio_esp_at_get_interface_description(void)
{
