 ...
 * 【補足説明】
 *
 *	タイムイベントが登録されていない時に高分解能タイマに設定する値は，
 *	ドリフト調整機能を持たない場合はHRTCNT_BOUNDであるのに対して，ドリ
 *	フト調整機能を持つ場合はTMAX_RELTIMをイベント時刻の伸縮率で割った
 *	値とHRTCNT_BOUNDの小さい方の値（このテストでは，ドリフト量を設定せ
 *	ず，HRTCNT_BOUND＞TMAX_RELTIMであるため，TMAX_RELTIMに一致）となる．
 *	そこで，HRTCNT_EMPTYをこの値に定義し，ドリフト調整機能の有無によら
 *	ず同じテストシーケンスが使えるようにする．
 *
 ...
 *		RETURN
 *	// タイムイベントがなくなる			... (A-1)
 *	8:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *	22:	wup_tsk(TASK1)
 *		RETURN
 *	23:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *	32:	wup_tsk(TASK1)
 *		RETURN
 *	33:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *	45:	wup_tsk(TASK1)
 *		RETURN
 *	46:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *	57:	wup_tsk(TASK1)
 *		RETURN
 *	58:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *	73:	wup_tsk(TASK1)
 *		RETURN
 *	74:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 *	// 高分解能タイマ割込みを強制的に入れる
 *	75:	DO(target_raise_hrt_int())
 *	== HRT_HANDLER ==					... (C-1)			// 時刻：1231
 *	// スプリアス割込み										// 時刻：1241
 *	76:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *		RETURN
 *	83:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 ...
 *	86:	adj_tim(-200)					... (A-4)	// ALM1の発生：4,000,001,511
 *	87:		[hook_hrt_set_event <- HRTCNT_BOUND]
 *	88:	slp_tsk()
 *	== HRT_HANDLER ==								// 時刻：4,000,001,312
 *	// スプリアス割込み								// 時刻：4,000,001,322
 *	89:		[hook_hrt_set_event <- 189U]
 *	== HRT_HANDLER ==								// 時刻：4,000,001,511
 *	== ALM1-8（8回目）==
 *	90:	assert(fch_hrt() == 4000001521U)			// 時刻：4,000,001,521
 *	91:	wup_tsk(TASK1)
 *		RETURN
 *	92:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 *	93:	END
 */
 ...
 *  カーネル内の直接操作
 *
 *  特殊なテスト（カーネル内の実行パスの確認，スプリアス割込みのテスト）
 *  を実施するために，カーネル内を直接操作する．
 */
