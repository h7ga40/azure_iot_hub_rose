...
/* 
 *		システム時刻管理機能のテスト(2)（USE_64BIT_HRTCNT版）
 *
 * 【テストの目的】
 *
 ...
 *		assert(fch_hrt() == (1LLU << 32) + 100U)			// 時刻：2^32+100
 ...
 *		assert(fch_hrt() == (1LLU << 32) + 150U)			// 時刻：2^32+150
 *	24:	get_tim(&systim)
 ...
 *	== ALM1-1（1回目）==
 *	27:	assert(fch_hrt() == (1LLU << 32) + 1000111U)	// 時刻：2^32+1000111
 *	// 非タスクコンテキストから各サービスコールを呼び出す
 *	28:	set_tim(1LLU << 32) -> E_CTX			... (A-1)
 *		get_tim(&systim) -> E_CTX				... (B-1)
 *		RETURN
 *	29:		[hook_hrt_clear_event]
 *	== TASK1（続き）==
 *	30:	DO(simtim_advance(89U))
 *		assert(fch_hrt() == (1LLU << 32) + 1000200U)	// 時刻：2^32+1000200
 *
 ...
 *		assert(fch_hrt() == (1LLU << 32) + 1500200U)	// 時刻：2^32+1500200
 *	// システム時刻が進んでいないことをチェック
 *	39:	get_tim(&systim)						... (B-4)
 *		assert(systim == (3LLU << 32) + 2000150U)
 *	// ここで時間が経過したことを想定
 *	40:	DO(simtim_advance(500100U))
 *		assert(fch_hrt() == (1LLU << 32) + 2000300U)	// 時刻：2^32+2000300
 *	// システム時刻が進んでいることをチェック
 ...
 *		assert(fch_hrt() == (1LLU << 32) + 2000320U)	// 時刻：2^32+2000320
 ...
 *		assert(fch_hrt() == (1LLU << 32) + 2000340U)	// 時刻：2^32+2000340
 *	// システム時刻が進んでいないことをチェック
 *	48:	get_tim(&systim)						... (B-5)
 *		assert(systim == (4LLU << 32))
 *	// ここで時間が経過したことを想定
 *	49:	DO(simtim_advance(500000U))
 *		assert(fch_hrt() == (1LLU << 32) + 2500340U)	// 時刻：2^32+2500340
 *	// システム時刻が進んでいないことをチェック
 *		get_tim(&systim)						... (B-5)
 *		assert(systim == (4LLU << 32))
 *	// ここで時間が経過したことを想定
 *	50:	DO(simtim_advance(500100U))
 *		assert(fch_hrt() == (1LLU << 32) + 3000440U)	// 時刻：2^32+3000440
 *	// システム時刻が進んでいることをチェック
 ...
 *	53:		[hook_hrt_clear_event]			// ALM1の停止
 *	54:		[hook_hrt_set_event <- 1001U]
 *	55:	DO(simtim_advance(10U))
 *		assert(fch_hrt() == (1LLU << 32) + 3000450U)	// 時刻：2^32+3000450
 *	56:	sta_alm(ALM2, 1000U)							// ALM2の発生：3001451
 *	// ここで時間が経過したことを想定
 *	57:	DO(simtim_advance(991U))
 *	== HRT_HANDLER ==									// 時刻：3001441
 *	== ALM1-2（2回目）==
 *	58:	assert(fch_hrt() == (1LLU << 32) + 3001451U)	// 時刻：2^32+3001451
 *	59:	adj_tim(+1000000)						... (C-8)(D-3)(E-2)
 *														// ALM2の発生：2001451
 *	60:	adj_tim(+1000000) -> E_OBJ				... (C-6)(E-3)
 *		RETURN
 *	== ALM2-1（1回目）==
 *	61:	assert(fch_hrt() == (1LLU << 32) + 3001451U)	// 時刻：2^32+3001451
 *		RETURN
 *	62:		[hook_hrt_clear_event]
 *	== TASK1（続き）==
 *	63:	DO(simtim_advance(49U))
 *		assert(fch_hrt() == (1LLU << 32) + 3001500U)	// 時刻：2^32+3001500
 *
 *	// (D-4)のテスト
 *	// ここで長時間経過したことを想定
 *	64:	DO(simtim_advance((1LLU << 32) - 3001500U - 100U))
 *	== TASK1（続き）==
 *	65:	assert(fch_hrt() == (1LLU << 33) - 100U)		// 時刻：2^33-100
 *	// システム時刻を設定する（この後の計算を楽にするため）
 *	66:	set_tim(5LLU << 32)
 *	// adj_timでシステム時刻を進める
 *	67:	adj_tim(200)								... (C-4)(D-4)(E-1)
 *	68:		[hook_hrt_clear_event]
 *	// システム時刻を確認
 *	69:	get_tim(&systim)
 *	70:	assert(systim == (5LLU << 32) + 200U)
 *
 *	// adj_timで繰り返しシステム時刻を戻すテスト
 *	// ここで少し時間が経過したことを想定
 *	71:	DO(simtim_advance(200U))
 *		assert(fch_hrt() == (1LLU << 33) + 100U)		// 時刻：2^33+100
 *	// システム時刻を設定する（この後の計算を楽にするため）
 *	72:	set_tim(6LLU << 32)
 *	// adj_timでシステム時刻を戻す
 *	73:	adj_tim(-200)								... (C-4)(D-1)(E-4)
 *	74:		[hook_hrt_clear_event]
 *	// システム時刻を確認
 *	75:	get_tim(&systim)
 *	76:	assert(systim == (6LLU << 32))
 *	// adj_timでシステム時刻をさらに戻す
 *	77:	adj_tim(-1000000)							... (C-4)(D-1)(E-4)
 *	78:		[hook_hrt_clear_event]
 *	79:	adj_tim(-1000000) -> E_OBJ					... (C-7)(E-5)
 *	80:	adj_tim(+1000000)							... (C-4)(D-2)(E-1)
 *	81:		[hook_hrt_clear_event]
 *	// システム時刻を確認
 *	// adj_tim(-200)を呼び出した時点でのシステム時刻になっているはず
 *	82:	get_tim(&systim)
 *		assert(systim == (6LLU << 32))
 *	// ここで時間が経過したことを想定
 *	83:	DO(simtim_advance(300U))
 *		assert(fch_hrt() == (1LLU << 33) + 400U)		// 時刻：2^33+400
 *	84:	get_tim(&systim)
 *		assert(systim == (6LLU << 32) + 100U)
 *	// adj_tim(0)の動作を確認
 *	85:	adj_tim(0)									... (C-4)(D-1)(E-6)
 *	86:		[hook_hrt_clear_event]
 *	87:	DO(simtim_advance(20U))
 *		assert(fch_hrt() == (1LLU << 33) + 420U)		// 時刻：2^33+420
 *	// システム時刻を確認
 *	88:	get_tim(&systim)
...
alarm1_handler(EXINF exinf)
{
	...
	case 1:
		check_point(27);
		check_assert(fch_hrt() == (1LLU << 32) + 1000111U);
	...
	case 2:
		check_point(58);
		check_assert(fch_hrt() == (1LLU << 32) + 3001451U);
...
task1(EXINF exinf)
{
	...
	check_assert(fch_hrt() == (1LLU << 32) + 100U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 150U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 1000200U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 1500200U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 2000300U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 2000320U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 2000340U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 2500340U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 3000440U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 3000450U);
	...
	check_assert(fch_hrt() == (1LLU << 32) + 3001500U);

	check_point(64);
	simtim_advance((1LLU << 32) - 3001500U - 100U);

	check_point(65);
	check_assert(fch_hrt() == (1LLU << 33) - 100U);
	...
	check_assert(fch_hrt() == (1LLU << 33) + 400U);
	...
	check_assert(fch_hrt() == (1LLU << 33) + 420U);
