 ...
 * 
 *  Copyright (C) 2012-2016 by Embedded and Real-Time Systems Laboratory
 ...
 *  をテストしている．このテストの際の情報を，以下の2つのメッセージ（1
 *  行目の数値はターゲットにより異なる）で出力している．
 *
 ...
 * 【テスト項目】
 *
 *	(A) ref_tskでの拡張サービスコールのネストレベルの参照［NGKI1243］
 ...
 *	TASK3: システムタスク．対象タスクに対して状態参照するためのタスク
 *	ALM1: 拡張サービスコールを呼び出す
 *	EXTSVC1: 拡張サービスコール
 ...
 *
 *	== TASK1（優先度：10）==
 *	1:	assert(TMAX_FNCD == TFN_EXTSVC3)					... (J)
 *		act_tsk(TASK2) -> E_OACV
 *		act_tsk(TASK3)
 *	== TASK3-1（優先度：9，1回目）==
 ...
 *															... (G-2)(G-4)(G-6)
 *	== EXTSVC1-1（1回目）==
 *	4:	state(false, false, false, false, false)
 *		ipm(TIPM_ENAALL)
 ...
 *	== EXTSVC1-1（続き）==
 *	10:	state(false, true, true, true, false)
 *		cal_svc(TFN_EXTSVC2, 0, 0, 0, 0, 0) -> E_PAR		... (G-1)(G-3)(G-5)
 *	== EXTSVC2-2（2回目）==
 *	11:	state(false, true, true, true, false)
 *		unl_cpu()
 *		chg_ipm(TIPM_ENAALL)
 *		RETURN(E_PAR)										... (H-1)(I-2)(I-4)
 *	== EXTSVC1-1（続き）==
 *	12:	state(false, false, true, true, false)
 *		ipm(TIPM_ENAALL)
 *		ena_dsp()											... (I-6)
 *	13:	RETURN(E_OK)
 *	== TASK1（続き）==
 *	14:	state(false, false, false, false, false)
 *		ipm(TIPM_ENAALL)
 *		cal_svc(TFN_EXTSVC3, 0, 0, 0, 0, 0) -> E_NOMEM		... (F-4)
 *	... ここで EXTSVC3 が実行されるが，この処理は手書きする
 *	== TASK1（続き）==
 *	15:	cal_svc(TFN_EXTSVC1, 0, 0, 0, 0, 0)
 *	== EXTSVC1-2（2回目）==
 *	16:	act_tsk(TASK2)										... (C-1)
 *	== TASK2（続き）==
 *	17:	act_tsk(TASK3)
 *	== TASK3-4（4回目）==
 *	18:	ref_tsk(TASK2, &rtsk)
 *		assert(rtsk.svclevel == 0)							... (A-1)
 *		ext_tsk()
 *	== TASK2（続き）==
 *	19:	cal_svc(0, 0, 0, 0, 0, 0) -> E_RSFN					... (F-1)
 *		cal_svc(TFN_EXTSVC3+1, 0, 0, 0, 0, 0) -> E_RSFN		... (F-2)
 *		cal_svc(TFN_EXTSVC0, 0, 0, 0, 0, 0) -> E_RSFN		... (F-3)
 *		cal_svc(TFN_EXTSVC1, 0, 0, 0, 0, 0)					... (B-1)
 *	== EXTSVC1-3（3回目）==
 *	20:	state(false, false, false, false, false)
 *		ipm(TIPM_ENAALL)
 *		assert(cdmid == TDOM_KERNEL)						... (D-1)
 *		act_tsk(TASK3)
 *	== TASK3-5（5回目）==
 *	21:	ref_tsk(TASK2, &rtsk)
 *		assert(rtsk.svclevel == 1)							... (A-2)
 *		ext_tsk()
 *	== EXTSVC1-3（続き）==
 *	22:	RETURN(E_OK)
 *	== TASK2（続き）==
 *	23:	cal_svc(TFN_EXTSVC3, 0, 0, 0, 0, 0) -> E_SYS		... (F-5)
 *	... ここで EXTSVC3 が実行されるが，この処理は手書きする
 *	== TASK2（続き）==
 *	24:	sta_alm(ALM1, 2 * TEST_TIME_CP) ... ALM1が実行開始するまで
 *		ext_tsk()
 *	== EXTSVC1-2（続き）==
 *	25:	RETURN(E_OK)
 *	== TASK1（続き）==
 *	26: slp_tsk()
 *	== ALM1 ==
 *	27:	cal_svc(0, 0, 0, 0, 0, 0) -> E_RSFN					... (F-1)
 *		cal_svc(TFN_EXTSVC3+1, 0, 0, 0, 0, 0) -> E_RSFN		... (F-2)
 *		cal_svc(TFN_EXTSVC0, 0, 0, 0, 0, 0) -> E_RSFN		... (F-3)
 *		cal_svc(TFN_EXTSVC1, 0, 0, 0, 0, 0)					... (B-2)
 *	== EXTSVC1-4（4回目）==
 *	28:	state(true, false, false, true, true)
 *		assert(cdmid == TDOM_KERNEL)						... (D-2)
 *		RETURN(E_OK)
 *	== ALM1（続き）==
 *	29:	state(true, false, false, true, true)
 *		wup_tsk(TASK1)
 *		RETURN
 *	== TASK1（続き）==
 *	30:	END
								...
								intptr_t par4, intptr_t par5, ID cdmid)
{
	...
	if (ercd < 0) {
		...
		if (SERCD(ercd) == -1) {
			 ...
			 *  を含むsyslog関数は使えないため，tSysLog_eSysLog_write
			 *  を関数呼出しする．
			 */
			{
				SYSLOG	logbuf;
				...
				logbuf.logpar[0] = (LOGPAR) msg;
				logbuf.logpar[1] = (LOGPAR) itron_strerror(ercd);
				logbuf.logpar[2] = (LOGPAR) rtsk.svclevel;
				(void) tSysLog_eSysLog_write(LOG_NOTICE, &logbuf);
			}
...
alarm1_handler(EXINF exinf)
{
	...
	check_point(29);
	check_state(true, false, false, true, true);
...
task1(EXINF exinf)
{
	...
	check_point(1);
	check_assert(TMAX_FNCD == TFN_EXTSVC3);
