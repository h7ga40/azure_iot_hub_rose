 ...
 */
ER_UINT
_serial_wri_dat(ID portid, const char *buf, uint_t len)
{
	SPCB	*p_spcb;
	bool_t	buffer_full;
	uint_t	wricnt = 0U;
 ...
 */
ER_UINT
_serial_rea_dat(ID portid, char *buf, uint_t len)
{
	SPCB	*p_spcb;
	bool_t	buffer_empty;
	uint_t	reacnt = 0U;
 ...
 */
ER
_serial_ctl_por(ID portid, uint_t ioctl)
{
	SPCB	*p_spcb;
 ...
 */
ER
_serial_ref_por(ID portid, T_SERIAL_RPOR *pk_rpor)
{
	SPCB	*p_spcb;
...
{
	return((ER_UINT) _serial_opn_por((ID) portid));
}
...
extsvc_serial_cls_por(intptr_t portid, intptr_t par2, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_cls_por((ID) portid));
}
							...
							intptr_t par4, intptr_t par5, ID cdmid)
{
	...
	if (!EXTSVC_PROBE_MEM_WRITE_SIZE(buf, len)) {
		ercd = E_MACV;				/* メモリアクセス権のチェック */
	}
	else {
		ercd = _serial_rea_dat((ID) portid, (char *) buf, (uint_t) len);
	}
	return(ercd);
}
							...
							intptr_t par4, intptr_t par5, ID cdmid)
{
	...
	if (!EXTSVC_PROBE_MEM_READ_SIZE(buf, len)) {
		ercd = E_MACV;				/* メモリアクセス権のチェック */
	}
	else {
		ercd = _serial_wri_dat((ID) portid, (const char *) buf, (uint_t) len);
	}
	return(ercd);
}
...
extsvc_serial_ctl_por(intptr_t portid, intptr_t ioctl, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_ctl_por((ID) portid, (uint_t) ioctl));
}
							...
							intptr_t par4, intptr_t par5, ID cdmid)
{
	...
	if (!EXTSVC_PROBE_MEM_WRITE(pk_rpor, T_SERIAL_RPOR)) {
		ercd = E_MACV;				/* メモリアクセス権のチェック */
	}
	else {
		ercd = (ER_UINT) _serial_ref_por((ID) portid,
