 ...
 *  サンプルプログラム(2)の本体
 *
 *  HRPカーネルの基本的な動作を確認するためのサンプルプログラム（TECS版）．
 *
 *  プログラムの概要:
 *
 *  ユーザインタフェースを受け持つメインタスク（MainTask）と，3つの並
 *  行実行されるタスク（Task1〜Task3），例外処理タスク（ExceptionTask），
 *  アラーム通知で起動されるタスク（AlarmTask）の6つのタスクを用いる．
 *  これらの他に，システムログタスクが動作する．また，周期ハンドラ，ア
 *  ラーム通知，割込みサービスルーチン，CPU例外ハンドラをそれぞれ1つ用
 ...
 *  'r' : rDomain1の3つの優先度（HIGH_PRIORITY，MID_PRIORITY，LOW_PRIORITY）
 *        のレディキューを回転させる．
 *  'c' : 周期ハンドラを動作開始させる．
 *  'C' : 周期ハンドラを動作停止させる．
...
eSampleTask_main(int_t subscript)
{
	...
	while (true) {
		...
		switch (c) {
		case '4':
			syslog(LOG_INFO, "#%d#cTask_activate(1)", tskno);
			SVC_PERROR(cTask_activate(1));
			break;
		case '5':
			syslog(LOG_INFO, "#%d#cTask_activate(2)", tskno);
			SVC_PERROR(cTask_activate(2));
			break;
		case '6':
			syslog(LOG_INFO, "#%d#cTask_activate(3)", tskno);
 ...
 *  割込みサービスルーチン
 *
 *  rDomain1のHIGH_PRIORITY，MID_PRIORITY，LOW_PRIORITY の各優先度のレ
 *  ディキューを回転させる．
 */
#ifdef INTNO1
...
{
	intno1_clear();
	SVC_PERROR(ciKernel_multiRotateReadyQueue(rDomain1, HIGH_PRIORITY));
	SVC_PERROR(ciKernel_multiRotateReadyQueue(rDomain1, MID_PRIORITY));
	SVC_PERROR(ciKernel_multiRotateReadyQueue(rDomain1, LOW_PRIORITY));
}
 ...
 *  アラーム通知で起動されるタスク
 *
 *  自ドメイン（rDomain1）のHIGH_PRIORITY，MID_PRIORITY，LOW_PRIORITY
 *  の各優先度のレディキューを回転させる．
 */
void
eAlarmTask_main(void)
{
...
eMainTask_main(void)
{
	...
	do {
		...
		case 'r':
			...
			SVC_PERROR(multiRotateReadyQueue(rDomain1, HIGH_PRIORITY));
			SVC_PERROR(multiRotateReadyQueue(rDomain1, MID_PRIORITY));
			SVC_PERROR(multiRotateReadyQueue(rDomain1, LOW_PRIORITY));
			break;
		case 'c':
		...
		case 'B':
			...
			break;
		case 'j':
			syslog(LOG_INFO, "#changeSystemOperationMode(SOM2)");
			SVC_PERROR(changeSystemOperationMode(SOM2));
			break;
		case 'J':
			syslog(LOG_INFO, "#changeSystemOperationMode(SOM1)");
			SVC_PERROR(changeSystemOperationMode(SOM1));
			break;
