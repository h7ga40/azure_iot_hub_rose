...
#else /* USE_TSKINICTXB */
	size_t		sstksz;			/* システムスタック領域のサイズ（丸めた値） */
	void		*sstk;			/* システムスタック領域の先頭番地 */
	size_t		ustksz;			/* ユーザスタック領域のサイズ（丸めた値） */
 ...
 *  		svclevel，bpriority，priority，wupque，raster，enater，p_lastmtx
 *  ・待ち状態（二重待ち状態を含む）で有効：
 *  		p_winfo
 ...
 */
extern TCB	*search_predtsk(SCHEDCB *p_schedcb);
...
update_schedtsk(void)
{
	if (schedcb_kernel.p_predtsk != NULL) {
		if (p_twdsched->p_predtsk != NULL
			...
			p_schedtsk = p_twdsched->p_predtsk;
		}
		else {
			p_schedtsk = schedcb_kernel.p_predtsk;
		}
	}
	else {
		if (p_twdsched->p_predtsk != NULL) {
			...
			p_schedtsk = p_idlesched->p_predtsk;
		}
	}
}
